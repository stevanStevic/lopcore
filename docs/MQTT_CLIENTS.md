...
This document explains the two MQTT client implementations in LopCore and helps you choose the right one for
your project.
...
The `esp-aws-iot` component (official AWS SDK for ESP32) uses CoreMQTT instead of ESP-MQTT for several
reasons:
...
AWS IoT requires X.509 certificates stored in hardware security modules. CoreMQTT integrates seamlessly with
corePKCS11:
...
Both clients have similar network performance. CoreMQTT has lower overhead for callback invocation in manual
mode.
...
Both have similar total memory usage. CoreMQTT's explicit state tracking uses ~1 KB more RAM but eliminates
hidden state in event loop.
