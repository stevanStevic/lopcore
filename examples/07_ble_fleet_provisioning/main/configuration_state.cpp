...
void ConfigurationState::update()
{
    switch (currentPhase_)
    {
        ...
        case Phase::FAILED_RETRY_WAIT: {
            LOPCORE_LOGI(TAG, "  Waiting %u ms before retry...", ctx_.config.config.ble_timeout_ms);
...
{
    uint32_t elapsed = (esp_timer_get_time() / 1000) - phaseStartTime_;
    return elapsed >= ms;
}
...
{
    currentPhase_ = phase;
    phaseStartTime_ = esp_timer_get_time() / 1000;
}
