...
public:
     ...
     */
    AwsFleetProvisioner(const AwsProvisioningConfig& config,
                       std::shared_ptr<TMqttClient> mqttClient)
        : config_(config)
    ...
    bool provision() override {
        status_ = ProvisioningStatus::IN_PROGRESS;
        ...
        if (!generateKeyPair()) {
            mqttClient_->disconnect();
            return failProvisioning(AwsProvisioningStep::GENERATING_KEYPAIR,
        ...
        if (!requestCertificateFromCsr()) {
            mqttClient_->disconnect();
            return failProvisioning(AwsProvisioningStep::SENDING_CSR,
                                   "Failed to send CSR to AWS");
        }
        ...
        if (!storeFinalCredentials()) {
            mqttClient_->disconnect();
            return failProvisioning(AwsProvisioningStep::STORING_CREDS,
                                   "Failed to store final credentials");
        }
...
private:
    ...
    bool subscribeToResponseTopics() {
        ...
        bool success = mqttClient_->subscribe(
            acceptCsrTopic.c_str(),
            [this](const char* payload, size_t length) {
                onCertificateAccepted(payload, length);
            }
        );

        success &= mqttClient_->subscribe(
            rejectCsrTopic.c_str(),
            [this](const char* payload, size_t length) {
        ...
        std::string acceptRegTopic = "$aws/provisioning-templates/" + templateName + "/provision/" + format + "/accepted";
        std::string rejectRegTopic = "$aws/provisioning-templates/" + templateName + "/provision/" + format + "/rejected";

        success &= mqttClient_->subscribe(
            acceptRegTopic.c_str(),
            [this](const char* payload, size_t length) {
    ...
    bool requestCertificateFromCsr() {
        ...
        if (!FleetProvisioningSerializer::generateCsrRequest(csrPem_, payload)) {
            return false;
        }
        
        return mqttClient_->publish(topic, 
    ...
    bool waitForCertificate() {
        ...
        for (int i = 0; i < 300; ++i) {  // 30 seconds / 100ms
    ...
    bool registerThing() {
        ...
        // Generate CBOR payload with template parameters
        ...
        std::vector<uint8_t> payload;
        if (!FleetProvisioningSerializer::generateRegisterThingRequest(params, payload)) {
            return false;
        }
    ...
    bool failProvisioning(AwsProvisioningStep step, const std::string& error) {
        ...
        status_ = ProvisioningStatus::FAILED;
        return false;
    }
    ...
    void onCertificateAccepted(const char* payload, size_t length) {
        ...
        CsrResponseData response;
        
        if (FleetProvisioningSerializer::parseCsrResponse(data, response)) {
        ...
        // Simple JSON extraction (production should use cJSON)
        std::string jsonStr(payload, length);
        size_t pemStart = jsonStr.find("\"certificatePem\":\"");
        if (pemStart != std::string::npos) {
            pemStart += 18;  // Length of "certificatePem":""
            size_t pemEnd = jsonStr.find("\"", pemStart);
            if (pemEnd != std::string::npos) {
                certificateReceived_ = jsonStr.substr(pemStart, pemEnd - pemStart);
                lastResult_.certificatePem = certificateReceived_;
            }
        }
        
        // Extract certificateId
        size_t idStart = jsonStr.find("\"certificateId\":\"");
        if (idStart != std::string::npos) {
            idStart += 17;
            size_t idEnd = jsonStr.find("\"", idStart);
            if (idEnd != std::string::npos) {
                certificateId_ = jsonStr.substr(idStart, idEnd - idStart);
            }
        }
#endif
    }
    ...
    void onRegisterThingAccepted(const char* payload, size_t length) {
        ...
        if (FleetProvisioningSerializer::parseRegisterThingResponse(data, thingName, deviceConfig)) {
        ...
        std::string jsonStr(payload, length);
        size_t nameStart = jsonStr.find("\"thingName\":\"");
        if (nameStart != std::string::npos) {
            nameStart += 13;  // Length of "thingName":""
            size_t nameEnd = jsonStr.find("\"", nameStart);
            if (nameEnd != std::string::npos) {
                lastResult_.deviceId = jsonStr.substr(nameStart, nameEnd - nameStart);
            }
        }
#endif
    }
