...
public:
    ...
    struct Config {
        ...
        size_t csrBufferSize = 2048;                            // CSR buffer size
        size_t certBufferSize = 2048;                           // Certificate buffer size
...
private:
    ...
    bool pkcs11StoreObject(const std::string& label,
                          const std::vector<uint8_t>& data,
                          uint32_t objectClass);
    bool pkcs11GetObject(const std::string& label,
                        std::vector<uint8_t>& data,
                        uint32_t objectClass) const;
    bool pkcs11ObjectExists(const std::string& label,
                           uint32_t objectClass) const;
     ...
     * 
     * Stores the signed certificate received from the cloud service into
     * PKCS11 secure storage for permanent use.
     * 
     * @param label PKCS11 label for the certificate (e.g., "device_cert")
     * @param certData Certificate data (PEM format)
     * @return true if storage succeeded
     */
    bool storeFinalCertificate(const std::string& label,
    ...
    /**
     * Store final device private key (permanent credentials)
     * 
     * Stores the private key into PKCS11 secure storage for permanent use.
     * 
     * @param label PKCS11 label for the key (e.g., "device_key")
     * @param keyData Private key data (PEM format)
     * @return true if storage succeeded
     */
    bool storeFinalPrivateKey(const std::string& label,
     ...
     * Check if claim certificate is loaded
     * 
     * @return true if claim cert and key are loaded
     */
    bool hasClaimCertificate() const;
    ...
    /**
     * Get claim certificate (for debugging/verification)
     * 
     * @return Claim certificate PEM, or std::nullopt if not loaded
     */
    std::optional<std::string> getClaimCertificate() const;
...
private:
    ...
    // PKCS11 operations (backend-agnostic)
    bool pkcs11StoreObject(const std::string& label,
                          const std::vector<uint8_t>& data,
                          uint32_t objectClass);
    bool pkcs11GetObject(const std::string& label,
    ...
    bool pkcs11ObjectExists(const std::string& label,
                           uint32_t objectClass);
    bool pkcs11DeleteObject(const std::string& label);
};
