...
AwsDataEndpointHandler::AwsDataEndpointHandler(const AwsDataEndpointConfig &config)
    : config_(config), lastResponse_("SUCCESS"), lastSuccess_(true)
{
                                            ...
                                            bool isComplete)
{
    ...
    // Parse and store — pass pointer directly, no heap copy needed here
    if (!parseAndStoreJson(data, length))
    {
...
size_t AwsDataEndpointHandler::getResponse(uint32_t sessionId, uint8_t *response, size_t maxLength)
{
    (void) sessionId; // Unused
...
bool AwsDataEndpointHandler::parseAndStoreJson(const uint8_t *data, size_t length)
{
    // Parse JSON without constructing an intermediate std::string
    ...
    const char *requiredFields[] = {"certificate", "private_key", "aws_endpoint", "root_ca",
