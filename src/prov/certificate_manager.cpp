...
bool CertificateManager::importClaimCertificate(const std::string &certPem, const std::string &keyPem)
{
    ...
    if (config_.usePkcs11 && pImpl_->p11SessionOpen)
    {
        ...
        if (!keyOk || !certOk)
        {
            LOPCORE_LOGE(TAG_CM, "Failed to store claim credentials in PKCS#11");
            return false;
        }
                                           ...
                                           uint32_t objectClass)
{
    ...
    {
        // Private key — parse and store via corePKCS11 PEM import path
        // The data vector is a null-terminated PEM string.
        CK_RV pkRv = provisionPrivateKey(pImpl_->p11Session, reinterpret_cast<const char *>(data.data()),
                                         data.size(), label.c_str());
        rv = pkRv;
    }
    ...
    : config_(config), pImpl_(std::make_unique<Impl>())
{
    ...
    LOPCORE_LOGI("CertManager", "  backend: %s",
                 config_.pkcs11Backend == Pkcs11Backend::AWS_CORE_PKCS11 ? "AWS corePKCS11"
                                                                         : "ESP-IDF PKCS11");
...
bool CertificateManager::generateCsr(const std::string &commonName, std::string &csrOut)
{
    ...
    {
        const char *pers = "lopcore_cert_manager";
        int ret = mbedtls_ctr_drbg_seed(&pImpl_->ctrDrbg, mbedtls_entropy_func, &pImpl_->entropy,
    ...
    ret = mbedtls_x509write_csr_pem(&csrCtx, csrBuffer.data(), csrBuffer.size(), mbedtls_ctr_drbg_random,
                                    &pImpl_->ctrDrbg);
    mbedtls_x509write_csr_free(&csrCtx);
...
bool CertificateManager::deleteClaimCredentials()
{
    LOPCORE_LOGI("CertManager", "Deleting claim credentials from memory");
    pImpl_->claimCertPem.clear();
    pImpl_->claimKeyPem.clear();
    pImpl_->hasClaimCert = false;
    // TODO: Delete from PKCS11 if config_.usePkcs11
    return true;
}

bool CertificateManager::deleteDeviceCredentials()
{
    LOPCORE_LOGI("CertManager", "Deleting device credentials (factory reset)");
    // TODO: Delete device cert + key from PKCS11
    LOPCORE_LOGW("CertManager", "deleteDeviceCredentials: PKCS11 deletion not yet implemented");
    return true;
}

bool CertificateManager::generateKeyPairAndCsr(const std::string &commonName, std::string &csrOut)
{
    LOPCORE_LOGI("CertManager", "generateKeyPairAndCsr: delegating to generateCsr");
    // When PKCS11 is enabled this will use a hardware-backed key pair.
    // Currently delegates to the mbedTLS software implementation.
    return generateCsr(commonName, csrOut);
}
...
{
    // TODO: Implement based on config_.pkcs11Backend
    LOPCORE_LOGW("CertManager", "pkcs11DeleteObject not yet implemented (label=%s)", label.c_str());
    return false;
}

int CertificateManager::pemToDer(const std::string &pem, std::vector<uint8_t> &der) const
{
    // TODO: implement PEM→DER conversion via mbedtls_pem_read_buffer
    LOPCORE_LOGW("CertManager", "pemToDer: not yet implemented");
    return -1;
}
