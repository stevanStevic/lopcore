...
std::optional<std::vector<uint8_t>> FleetProvisioningSerializer::generateRegisterThingRequest(
    ...
    if (serialNumber.empty()) {
        LOPCORE_LOGE("FleetProvSerializer", "Serial number is empty");
        return std::nullopt;
    }

    // Allocate buffer
    std::vector<uint8_t> buffer(ownershipToken.size() + serialNumber.size() + 256);
    ...
    // Create map with 2 key-value pairs
    ...
    if (cborRet == CborNoError) {
        // Parameters map with 1 entry (SerialNumber)
    ...
    if (cborRet == CborNoError) {
        cborRet = cbor_encode_text_string(&parametersEncoder, serialNumber.c_str(), serialNumber.length());
    }
...
std::optional<CsrResponseData> FleetProvisioningSerializer::parseCsrResponse(const std::vector<uint8_t>& payload) {
    return parseCsrResponse(payload.data(), payload.size());
}

std::optional<CsrResponseData> FleetProvisioningSerializer::parseCsrResponse(const uint8_t* payload, size_t length) {
    ...
    LOPCORE_LOGD("FleetProvSerializer", "  Ownership token length: %zu bytes", result.ownershipToken.length());
...
std::optional<std::string> FleetProvisioningSerializer::parseRegisterThingResponse(const std::vector<uint8_t>& payload) {
    return parseRegisterThingResponse(payload.data(), payload.size());
}

std::optional<std::string> FleetProvisioningSerializer::parseRegisterThingResponse(const uint8_t* payload, size_t length) {
    if (payload == nullptr || length == 0) {
...
std::optional<std::string> FleetProvisioningSerializer::parseRegisterThingResponse(const std::vector<uint8_t>&) {
    LOPCORE_LOGE("FleetProvSerializer", "CBOR support not enabled (CONFIG_LOPCORE_PROV_AWS_CBOR)");
    return std::nullopt;
}
