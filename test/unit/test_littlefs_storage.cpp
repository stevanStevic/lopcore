/**
 * @file test_littlefs_storage.cpp
 * @brief Unit tests for LittleFsStorage
 */
...
// Test fixture for LittleFsStorage
class LittleFsStorageTest : public ::testing::Test
{
protected:
    std::unique_ptr<LittleFsStorage> storage;
    std::string testPath;

    void SetUp() override
    {
        // Create temporary directory for tests
        testPath = "/tmp/lopcore_littlefs_test";
        mkdir(testPath.c_str(), 0755);

        auto config =
            storage::LittleFsConfig().setBasePath(testPath).setPartitionLabel("test").setFormatIfFailed(true);
...
TEST_F(LittleFsStorageTest, Constructor_InitializesSuccessfully)
{
    ASSERT_NE(storage, nullptr);
    EXPECT_EQ(storage->getType(), StorageType::LITTLEFS);
    EXPECT_EQ(storage->getBasePath(), testPath);
}

TEST_F(LittleFsStorageTest, Write_ValidStringData_ReturnsTrue)
{
...
TEST_F(LittleFsStorageTest, Read_ExistingKey_ReturnsData)
{
...
TEST_F(LittleFsStorageTest, Read_NonExistentKey_ReturnsNullopt)
{
...
TEST_F(LittleFsStorageTest, Exists_ExistingKey_ReturnsTrue)
{
...
TEST_F(LittleFsStorageTest, Exists_NonExistentKey_ReturnsFalse)
{
...
TEST_F(LittleFsStorageTest, Remove_ExistingKey_ReturnsTrue)
{
...
TEST_F(LittleFsStorageTest, ListKeys_MultipleFiles_ReturnsAllKeys)
{
...
TEST_F(LittleFsStorageTest, WriteBinary_ValidData_ReturnsTrue)
{
...
TEST_F(LittleFsStorageTest, ReadBinary_ExistingKey_ReturnsData)
{
...
TEST_F(LittleFsStorageTest, Write_BinaryFilePreservesExtension)
{
    std::string key = "acc_raw_123.bin";
    ...
    // List files and check the extension is preserved
    auto files = storage->listKeys("");
    EXPECT_NE(std::find(files.begin(), files.end(), key), files.end());
}

// ============================================================================
// C Array Pointer Tests
// ============================================================================

TEST_F(LittleFsStorageTest, Write_CArrayPointer_Success)
{
...
TEST_F(LittleFsStorageTest, Write_NullPointer_ReturnsFalse)
{
...
TEST_F(LittleFsStorageTest, Write_ZeroLength_ReturnsFalse)
{
...
TEST_F(LittleFsStorageTest, Write_LargeCArray_Success)
{
...
// ============================================================================
// Space and Size Tests
// ============================================================================

TEST_F(LittleFsStorageTest, HasSpace_SmallAmount_ReturnsTrue)
{
...
TEST_F(LittleFsStorageTest, GetFileSize_ExistingFile_ReturnsCorrectSize)
{
...
TEST_F(LittleFsStorageTest, GetFileSize_NonExistentFile_ReturnsNullopt)
{
...
TEST_F(LittleFsStorageTest, GetFileSize_BinaryFile_ReturnsCorrectSize)
{
    std::string key = "binary_size.bin";
...
TEST_F(LittleFsStorageTest, ListKeysByPattern_SingleMatch_ReturnsOne)
{
...
TEST_F(LittleFsStorageTest, ListKeysByPattern_Wildcard_ReturnsMultiple)
{
    storage->write("acc_raw_1.bin", "data1");
    storage->write("acc_raw_2.bin", "data2");
...
TEST_F(LittleFsStorageTest, ListKeysByPattern_NoMatches_ReturnsEmpty)
{
    storage->write("file1.txt", "data1");
    storage->write("file2.txt", "data2");
...
TEST_F(LittleFsStorageTest, ListKeysByPattern_PrefixWildcard_Matches)
{
...
TEST_F(LittleFsStorageTest, ListKeysByPattern_SuffixOnly_Matches)
{
    storage->write("file1.bin", "data1");
    storage->write("file2.bin", "data2");
    storage->write("file3.txt", "data3");

    auto files = storage->listKeysByPattern("", "*.bin");
...
TEST_F(LittleFsStorageTest, RemoveByPattern_MultipleMatches_DeletesAll)
{
...
TEST_F(LittleFsStorageTest, RemoveByPattern_NoMatches_ReturnsZero)
{
...
TEST_F(LittleFsStorageTest, RemoveByPattern_ExactMatch_DeletesOne)
{
...
TEST_F(LittleFsStorageTest, RemoveByPattern_LargeNumberOfFiles_DeletesAll)
{
    // Create 20 files
    for (int i = 0; i < 20; i++)
    {
...
TEST_F(LittleFsStorageTest, ListDetailed_MultipleFiles_ReturnsDetails)
{
...
TEST_F(LittleFsStorageTest, ListDetailed_EmptyDirectory_ReturnsEmpty)
{
    auto files = storage->listDetailed("");
...
TEST_F(LittleFsStorageTest, DisplayStats_NoFiles_DoesNotCrash)
{
    // Just verify it doesn't crash
    EXPECT_NO_THROW(storage->displayStats());
}

TEST_F(LittleFsStorageTest, DisplayStats_WithFiles_DoesNotCrash)
{
    storage->write("file1.txt", "data");
    storage->write("file2.txt", "more data");
...
TEST_F(LittleFsStorageTest, Write_LargeFile_Success)
{
    std::string key = "large_file.dat";
    std::string largeData(10000, 'X'); // 10KB
...
TEST_F(LittleFsStorageTest, Write_ExistingKey_Overwrites)
{
...
TEST_F(LittleFsStorageTest, Write_EmptyString_Success)
{
...
TEST_F(LittleFsStorageTest, Write_SpecialCharacters_Success)
{
...
TEST_F(LittleFsStorageTest, Concurrent_MultipleWrites_AllSucceed)
{
